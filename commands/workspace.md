...
2. If no saved default and the user did not state a choice, derive a proposed branch and path, then ask: **main**, or **`<branch>` at `<path>`** (never a bare label `worktree`).
